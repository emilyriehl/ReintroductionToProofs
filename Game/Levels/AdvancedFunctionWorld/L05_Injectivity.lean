...
Statement Bool.not_injective : ∀ x y : Bool, not x = not y → x = y := by
  ...
  cases x <;> cases y
  rfl
  Hint (hidden := true) "This case is considerably more difficult than the previous one because your goal is to prove that `false = true`. But you may recall from Boolean world that we can prove `¬ (false = true) := Bool.noConfusion` and `¬ (true = false) := Bool.noConfusion`. This suggests that the only way we will be able to prove `false = true` is by finding a contradition among our hypothesis. To set up this proof strategy, use the tactic `exfalso`."
  exfalso
  Hint (hidden := true) "Now where does the contradiction come from? We have exactly one hypothesis, namely `{p} : not false = not true`. Recall that `not false = true` and `not true = false`, with both of these equalities by definition. Thus `{p}` is also a proof of `true = false`. You can add this to your context by `have eq : true = false := p`."
  have eq : true = false := p
  Hint (hidden := true) "To get the contradiction, you should now add a proof of the negation of `{eq}`. You can do this with `have neq : ¬ (true = false) := Bool.noConfusion`."
  have neq : ¬ (true = false) := Bool.noConfusion
  Hint (hidden := true) "Revisit Negation World to remind yourself how to get an element of `False` from `{eq}` and `{neq}`."
  exact neq eq
