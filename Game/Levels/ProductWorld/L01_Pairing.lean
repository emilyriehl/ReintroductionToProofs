...
Introduction "Given an element `a : A` and an element `b : B` there is a corresponding element of the product type `A × B` that is denoted by `⟨a , b⟩ : A × B`. This element should be thought of as an *ordered pair* whose first component is the element `a : A` and whose second element is the element `b : B`.

This can be input directly by typing `exact ⟨a,b⟩' using '\\\\<' and '\\\\>' to type the angle braces; note the spacing around the angle braces and the commas is not important.
