...
Note `exact p` or `exact q` won't work as these are proofs of different propositions, namely the proposition `P` or the proposition `Q`.
...
Type `left` to tell Lean you'd like to prove the left proposition or type `right` to tell Lean you'd like to prove the right proposition.
"
...
Conclusion "If this is your first time solving this level, delete your code and prove the same theorem another way. Alternatively, you can prove `P ∨ Q` by applying either of the implications `Or.inl : P → P ∨ Q` and `Or.inr : Q → P ∨ Q` that are now in the library."
