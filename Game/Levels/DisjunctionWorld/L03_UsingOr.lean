...
Introduction "Given a hypothesis `h : P ∨ Q` we know that *either* `P` or `Q` is true (or possibly both are true) but we don't know *which* of `P` or `Q` is true. it suffices to supply a proof of either `P` or of `Q`.
...
Statement {P Q : Prop} : P ∨ Q → Q ∨ P := by
  ...
  intro h
  Hint "After introducing the hypothesis `{h} : {P} ∨ {Q}`, type `cases h` to ask Lean to consider both cases: the first being that `P` is true and the second being that `Q` is true."
  rcases h with q | p
  ...
  apply Or.inr
  Hint "You can solve the goal by explicitly referring to the named hypothesis that Lean supplied or by typing `assumption` to indicate that the goal matches an assumption."
  assumption
