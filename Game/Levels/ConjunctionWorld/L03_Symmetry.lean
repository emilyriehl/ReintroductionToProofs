...
Introduction "If `P ∧ Q` is true, then `Q ∧ P` is too. In other words, `P ∧ Q` implies `Q ∧ P`, as is expressed by the proposition `P ∧ Q → Q ∧ P` which is implicitly parenthesized as `(P ∧ Q) → (Q ∧ P)`."
...
Conclusion "In fact `P ∧ Q` is true if and only if `Q ∧ P` is true, as we will now demonstrate."
