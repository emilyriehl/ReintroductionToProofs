...
then there is a function `ℕ → A`.

The idea is we define the function `f : ℕ → A` by specifying that
* `f 0 = f_0` and
...
Statement : ∃ f : ℕ → ℕ, (f 0 = 0) ∧ (∀ n : ℕ, f (succ n) = n) := by
  ...
  constructor
  rfl
  Hint (hidden := true) "Start with `intro` to prove the universally quantified statement."
  intro n
  rfl
