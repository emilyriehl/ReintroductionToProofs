...
Lean has another tactic called `apply`, which can be used in the presence of a function to transform the goal. If `f : A → B` and the goal is to define an element of type `B` then `apply f` will update the goal to ask instead for an element of type `A`.

This asks Lean to solve the initial goal by applying the function `f` to the element you provide next.
"
...
Conclusion "While the tactics used in this level and the previous one are different, both this level and the previous one ultimately tell Lean to use the element `f a : B` built by applying the function `f` to the element `a`."
