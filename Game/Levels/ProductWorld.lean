...
In this world, we'll meet another binary type forming operation. Given two types `A` and `B`, there is a type `A × B` called the *product type* whose elements `p : A × B` should be thought of as encoding ordered pairs of elements, one from `A` and one from `B`.
